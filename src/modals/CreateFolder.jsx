...
// const folderColors = ["#FFD156", "#F23B31", "#F6722A", "#00A254", "#00948E", "#2880CB", "#A759C2", "#DA49B0", "#D9D9D9", "#FFBCB4", "#FFC08B", "#7AD596", "#68D2CD", "#83C6F3", "#DEABF2", "#FFA6E4"];

// const limitPerPage = 6;

// const CreateFolder = ({ isOpen, onClose, callback, folderData, parentFolderId , isEdit = false}) => {
...
//           folderName: isEdit && folderData?.folder_name ? decrypt(folderData?.folder_name) : "",
//           folderColor: isEdit && folderData?.folder_color ? decrypt(folderData?.folder_color) : "",
//           isAdvanced: false,
//           addType: "user",
//           userIds: [],
//           circleIds: [],
...
//           await callback({ ...values, userIds: values?.userIds?.map((user) => user.user_id) }, resetForm);
//           setSubmitting(false);
//         }}
//       >
//         {({ handleSubmit, setFieldValue, values }) => {
//           // Define queries inside Formik to access values.addType
//           const folderQueryConfig = createQueryConfig({
//             queryKey: ["folders-main", currentPage, searchTerm],
//             queryFn: fetchFolders,
//             enabled: values.addType === "folder", // Only fetch when addType is folder
//           });

//           const groupQueryConfig = createQueryConfig({
//             queryKey: ["groups", currentPage, searchTerm],
//             queryFn: fetchGroups,
//             enabled: values.addType === "circle", // Only fetch when addType is circle
//           });

//           const folderuserQueryConfig = createQueryConfig({
//               queryKey: ["folder-user",isEdit],
...
//                 setFilteredItems([]); // Clear filtered items for user search
//               }
//             }
//           };

//           // Update filteredItems when data or searchTerm changes
//           useEffect(() => {
//             let data = [];
...
//                       )
//                     ) : values.addType !== "user" &&
//                       (isFoldersLoading || isGroupsLoading || isFoldersFetching || isGroupsFetching) ? (
//                       <div className="py-2">
//                         <Spinner size={36} />
//                       </div>
//                     ) : values.addType !== "user" && filteredItems?.length > 0 ? (
...
//                           </div>
//                           {(values.addType === "folder" &&
//                             values.folderIds.includes(item.folder_id)) ||
//                             (values.addType === "circle" &&
...
//                                   setFieldValue(
//                                     "userIds",
//                                     values.userIds.filter((u) => u.user_id !== user.user_id))}
//                               >
//                                 <RxCross2 className="cursor-pointer" />
//                               </button>
//                             </div>
            ...
            >
              ...
              <div className="flex flex-wrap gap-4 py-3 px-5 items-center justify-between">
                {folderColors.map((color) => (
                  <div
                    key={color}
                    className={`rounded-full cursor-pointer border-2 hover:border-slate-300 flex items-center justify-center ${
                      values.folderColor === color
                        ? "border-slate-300"
                        : "border-transparent"
                    }`}
                    style={{ padding: "1px" }}
